...
with DAG(
   ...
   # Task 4: Copy Data from S3 to Snowflake for "pre"
    ...
    task_id='dbt_run',
    bash_command=f"cd {DBT_PROJECT_DIR} && dbt run --profiles-dir {DBT_PROFILE_DIR}",
    dag=dag
    )
    ...
    [copy_from_s3_sub, copy_from_s3_num, copy_from_s3_tag, copy_from_s3_pre] >> dbt_run >> dbt_test